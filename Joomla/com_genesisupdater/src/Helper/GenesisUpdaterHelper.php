...
namespace Joomla\Component\Genesisupdater\Administrator\Helper;
...
use Joomla\Component\Genesisupdater\Administrator\Platform\PlatformRegistry;
use Joomla\Component\Genesisupdater\Administrator\Platform\PlatformInterface;
...
class GenesisUpdaterHelper
{
    ...
    public static function generateAllChangelogs(): array
    {
        ...
        $query = $db->getQuery(true)
            ->select($db->quoteName('id'))
            ->from($db->quoteName('#__genesisupdater_changelogs'))
            ->where($db->quoteName('state') . ' = 1');
        $db->setQuery($query);
    ...
    {
        $db = Factory::getContainer()->get(\Joomla\Database\DatabaseInterface::class);
        $query = $db->getQuery(true)->select('*')->from($db->quoteName('#__genesisupdater_bundles'))->where($db->quoteName('id') . ' = :id')->bind(':id', $bundleId, \Joomla\Database\ParameterType::INTEGER);
    ...
    {
        $db = Factory::getContainer()->get(\Joomla\Database\DatabaseInterface::class);
        $query = $db->getQuery(true)->select($db->quoteName('id'))->from($db->quoteName('#__genesisupdater_bundles'))->where($db->quoteName('state') . ' = 1');
