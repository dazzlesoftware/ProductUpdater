...
class WordPressPlatform extends AbstractPlatform
{
    ...
    public function generate(array $product): string
    {
        ...
        if ($changelogMode === 'generated') {
            $changelogUrl = GenesisUpdaterHelper::getPublicUrl(GenesisUpdaterHelper::getProductChangelogFilePath($product, $this)) ?? '';
        }
        if ($changelogUrl !== '') { $data['changelog_url'] = $changelogUrl; }

        $changelog = GenesisUpdaterHelper::loadChangelogForProduct((int) ($product['id'] ?? 0));
        $sections = [];
