 ...
 */
namespace Joomla\Component\Genesisupdater\Site\Model;
...
class DownloadsModel extends BaseDatabaseModel
{
    public function getItems(int $categoryId = 0): array
    {
		...
		$query = $db->getQuery(true)
			...
			->from($db->quoteName('#__genesisupdater_products', 'p'))
			->leftJoin($db->quoteName('#__categories', 'c') . ' ON ' . $db->quoteName('c.id') . ' = ' . $db->quoteName('p.catid') . ' AND ' . $db->quoteName('c.extension') . ' = ' . $db->quote('com_genesisupdater'))
	...
	{
		$db = $this->getDatabase();
		$query = $db->getQuery(true)->select($db->quoteName('entries'))->from($db->quoteName('#__genesisupdater_changelogs'))->where($db->quoteName('product_id') . ' = ' . $productId)->where($db->quoteName('state') . ' = 1')->order($db->quoteName('id') . ' DESC');
    ...
    private function getChangelogUrl(array $product, array $row): string
    {
        ...
        $base = rtrim((string) ComponentHelper::getParams('com_genesisupdater')->get('base_url', ''), '/');
        if ($base === '') return '';
