...
class ProductTable extends Table
{
    ...
    public function __construct(DatabaseInterface $db, ?DispatcherInterface $dispatcher = null)
    {
        ...
        parent::__construct('#__productupdater_products', 'id', $db, $dispatcher);
    }
    ...
    public function check()
    {
        if (trim((string) $this->title) === '') {
            $this->setError(Text::_('COM_GENESISPRODUCTUPDATER_ERROR_TITLE_REQUIRED'));

            return false;
        }

        if (trim((string) $this->element) === '') {
            $this->setError(Text::_('COM_GENESISPRODUCTUPDATER_ERROR_ELEMENT_REQUIRED'));
        ...
        if (!in_array((string) $this->platform, ['joomla', 'wordpress', 'mobile', 'fab'], true)) {
			$this->setError(Text::_('COM_GENESISPRODUCTUPDATER_ERROR_PLATFORM_REQUIRED'));
