...
Genesis Updater lets you manage one "Product" per item you ship (a Joomla
...
Change the folder any time under **Genesis Updater > Settings**.

= Changelogs =

**Genesis Updater > Changelogs** is a separate screen from Products: pick
...
Platforms are plain PHP classes extending `Genesis_Updater_Platform_Base`
(`includes/platforms/class-genesis-updater-platform-base.php`... see
`class-genesis-updater-platform-joomla.php` for a full example). Drop a new class in
`includes/platforms/`, require it from the main plugin file, and register
it with the `genesis_updater_register_platforms` filter. It then automatically appears
...
* New row fields Build Number / URL (iOS) / URL (Android) / Force Update / Release Notes, shown only for rows on a platform that opts in via Genesis_Updater_Platform_Base::supports_mobile_fields() (Requires/Tested/Requires PHP are hidden on those rows instead, since mobile releases don't gate on a platform-core version).

= 1.2.0 =
* The Requires/Tested up to/Requires PHP fields on each version row now show/hide and relabel per-row based on that row's Platform (e.g. "Requires Joomla Version" vs "Requires WordPress Version"). Platforms can opt out entirely via Genesis_Updater_Platform_Base::supports_compatibility_fields().

= 1.1.1 =
* WordPress-safe `genesis_up_*` identifiers are used for post types and taxonomies; full `genesis_updater_*` identifiers are used everywhere without WordPress length restrictions.
