 ...
 * to disk.
 */
class Product_Updater_File_Writer {
	...
	public function get_settings() {
		...
		$settings = get_option( 'product_updater_settings', array() );
	...
	public function get_file_path( Product_Updater_Platform_Base $generator, array $product ) {
		return $this->get_base_dir() . '/' . str_replace( '/', DIRECTORY_SEPARATOR, $generator->get_subpath( $product ) );
	}

	public function get_file_url( Product_Updater_Platform_Base $generator, array $product ) {
	...
	public function get_file_info( Product_Updater_Platform_Base $generator, array $product ) {
	...
	public function get_changelog_file_path( Product_Updater_Platform_Base $generator, array $product ) {
		return $this->get_base_dir() . '/' . str_replace( '/', DIRECTORY_SEPARATOR, $generator->get_changelog_subpath( $product ) );
	}

	public function get_changelog_file_url( Product_Updater_Platform_Base $generator, array $product ) {
		$base_url = $this->get_base_url();
		if ( '' === $base_url ) {
			return '';
		}
	...
	public function get_changelog_file_info( Product_Updater_Platform_Base $generator, array $product ) {
	...
	private function write_file( $path, $contents ) {
		...
		if ( ! wp_mkdir_p( $dir ) ) {
			return new WP_Error( 'product_updater_mkdir_failed', sprintf( __( 'Could not create directory: %s', 'genesis-product-updater' ), $dir ) );
		}

		$result = @file_put_contents( $path, $contents ); // phpcs:ignore WordPress.PHP.NoSilencedErrors
		if ( false === $result ) {
			return new WP_Error( 'product_updater_write_failed', sprintf( __( 'Could not write file: %s', 'genesis-product-updater' ), $path ) );
		}
	 ...
	 */
	public function write( Product_Updater_Platform_Base $generator, array $product, array $rows ) {
	 ...
	 * failure, or the absolute path written on success.
	 */
	public function write_changelog( Product_Updater_Platform_Base $generator, array $product, array $entries ) {
	...
	public function generate_product( $post_id ) {
		...
		foreach ( $byplat as $slug => $platform_rows ) {
			...
			if ( ! $generator ) {
				$results[ $slug ] = new WP_Error( 'product_updater_unknown_platform', sprintf( __( 'Unknown platform "%s"', 'genesis-product-updater' ), $slug ) );
				continue;
			}
	...
	public function generate_changelog_for_product( $product_id ) {
		...
		$entries = Product_Updater_Changelog::get_entries_for_product( $product_id );
		...
		foreach ( Product_Updater_Generator_Manager::instance()->get_platforms() as $slug => $generator ) {
