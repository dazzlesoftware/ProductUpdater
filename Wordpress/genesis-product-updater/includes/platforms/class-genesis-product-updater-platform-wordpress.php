 ...
 */
class Product_Updater_Platform_WordPress extends Product_Updater_Platform_Base {
	...
	public function generate( array $product, array $rows ) {
		...
		if ( 'generated' === $changelog_mode ) {
			$base_url = Product_Updater_File_Writer::instance()->get_base_url();
			if ( ! empty( $base_url ) ) {
		...
		$changelog_html = Product_Updater_Changelog::get_changelog_html_for_product( $product['post_id'] );
		$sections       = array();
	...
	public function generate_changelog( array $product, array $entries ) {
		...
		$data['changelog_html'] = Product_Updater_Changelog::render_html( $entries );
...
add_filter(
	...
	function ( $platforms ) {
		$platforms['wordpress'] = new Product_Updater_Platform_WordPress();
		return $platforms;
	}
);
