 ...
 */
class Product_Updater_Platform_Fab extends Product_Updater_Platform_Base {
	...
	public function generate( array $product, array $rows ) {
		...
		if ( '' === $changelog && ! empty( $product['post_id'] ) ) {
			$entries = Product_Updater_Changelog::get_entries_for_product( $product['post_id'] );
...
add_filter( 'product_updater_register_platforms', function ( $platforms ) { $platforms['fab'] = new Product_Updater_Platform_Fab(); return $platforms; } );
